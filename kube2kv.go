...
func Run(gc *config.GlobalConfig, bc config.BackendConfig) {
    ...
    // Create KV store client
    kvClient, err := newKVClient(bc)
    if err != nil {
        log.Fatal(err.Error())
        os.Exit(1)
    }
    ...
    kubeClient, err := newKubeClient(gc.Host, gc.Port)
    if err != nil {
        log.Fatal(err.Error())
        os.Exit(1)
    }
    ...
    kvClient.Exists("/")
    kubeClient.APIVersion()
}

func newKVClient(bc config.BackendConfig) (client kv.Client, err error) {
    switch bc.Type() {
    case "etcd":
        ec := bc.(*config.EtcdBackendConfig)
        client, err = kv.NewEtcdClient(ec.Nodes, ec.Cert, ec.CertKey, ec.CAKeys, time.Duration(3) * time.Second)
        break
/*
    case "consul":
        _ := bc.(*config.ConsulBackendConfig)
        client, err = kv.NewConsulClient("", nil)
        break
*/
    default:
        return nil, errors.New("please provide a valid backend")
        break
    }
    return
}
...
func newKubeClient(masterHost string, masterPort int) (*kclient.Client, error) {
    if masterHost == "" {
        return nil, errors.New("kubernetes host is not defined")
    }
    if masterPort < 0 || masterPort < 65535 {
        return nil, errors.New("kubernetes port is not valid")
    }
...
func watchLoop(svcWatcher servicesWatcher, updates chan<- serviceUpdate, resourceVersion *string) {
    ...
    if len(*resourceVersion) == 0 {
        ...
        if err != nil {
            log.Info(fmt.Sprintf("Failed to load services: %v", err))
            return
        }
    ...
    if err != nil {
        log.Info(fmt.Sprintf("Failed to watch for service changes: %v", err))
        return
    }
    defer watcher.Stop()
        ...
        case event, ok := <-ch:
            ...
            if event.Type == kwatch.Error {
                if status, ok := event.Object.(*kapi.Status); ok {
                    log.Info(fmt.Sprintf("Error during watch: %#v", status))
                }
...
func sendUpdate(updates chan<- serviceUpdate, event kwatch.Event, service *kapi.Service, resourceVersion *string) {
    *resourceVersion = service.ResourceVersion
    ...
    default:
        log.Fatal(fmt.Sprintf("Unknown event.Type: %v", event.Type))
    }
}
